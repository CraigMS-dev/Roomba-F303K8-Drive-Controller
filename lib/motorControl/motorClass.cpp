...
tachoWheel::tachoWheel(int pinA, int pinB, int pinA_offset, int pinB_offset){
    ...
    void tachoWheel::encoderTick()
    {
        ...
        if(PulseCounter >= AmountOfReadings){  // If counter for amount of readings reach the set limit:
            ...
            currentState         = (digitalRead(_PinA) << 1) | digitalRead(_PinB);
            increment            = QEM[lastState][currentState];
            
            
            if(wheelSpeedDistanceDif > 0){
                direction = 1;
            }else if(wheelSpeedDistanceOld < 0){
                direction = -1;
            }else{
                direction = 0;
            }
            ...
            wheelSpeedDistanceCurrent += increment;
            wheelSpeedDistanceDif      = wheelSpeedDistanceCurrent - wheelSpeedDistanceOld;

            PeriodSum += PeriodBetweenPulses;  // Add the periods so later we can average.
            
            lastState = currentState;
            wheelSpeedDistanceOld = wheelSpeedDistanceCurrent;
        }
    }
    /*void tachoWheel::encoderTickB()
    {
    ...
    void tachoWheel::sendInfo()
    {
        ...
        //Serial.print("\t");
        //Serial.print(wheelSpeedDistanceOld);
        //Serial.print("\t");
        Serial.print(wheelSpeedDistanceCurrent);
        //Serial.println();
    }
